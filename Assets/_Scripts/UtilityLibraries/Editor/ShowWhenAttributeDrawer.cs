...
namespace Com.StellarPixels.UtilityLibraries.Editor
{
	...
	public class ShowWhenDrawer : PropertyDrawer
	{
		...
		public override void OnGUI(Rect _position_, SerializedProperty _property_, GUIContent _label_)
		{
			...
			switch (conditionField_.propertyType)
			{
				...
				case SerializedPropertyType.Enum:
					...
					{
						if (paramEnum_ != null && !CheckSameEnumType(
																	new[] { paramEnum_.GetType() },
																	_property_
																		.serializedObject
																		.targetObject
																		.GetType(),
																	conditionField_.name))
						{
					...
					{
						if (!CheckSameEnumType(
												paramEnumArray_.Select(_x_ => _x_.GetType()),
												_property_.serializedObject.targetObject.GetType(),
												conditionField_.name))
						{
							ShowError(_position_, _label_, "Enum Types doesn't match");
							return;
				...
				case SerializedPropertyType.Float:
					...
					if (stringValue_.StartsWith("==", StringComparison.Ordinal))
					{
					...
					} else if (stringValue_.StartsWith("!=", StringComparison.Ordinal))
					{
						var value_ = GetValue(stringValue_, "!=");
						if (value_ == null)
						{
		...
		/// </summary>
		private static bool IsEnum([CanBeNull] object _obj_) => _obj_?.GetType().IsEnum == true;

		/// <summary>
		/// Return if all the objects are enums and not null.
		/// </summary>
		private static bool IsEnum([CanBeNull] object[] _obj_)
			=> _obj_?.All(_o_ => _o_.GetType().IsEnum) == true;

		/// <summary>
		/// Check if the field with name "fieldName" has the same class as the "checkTypes" classes through reflection.
		/// </summary>
		private static bool CheckSameEnumType(IEnumerable<Type> _checkTypes_, [NotNull] Type _classType_, string _fieldName_)
		{
		...
		/// </summary>
		private static float? GetValue([NotNull] string _content_, [NotNull] string _remove_)
		{
			var removed_ = _content_.Replace(_remove_, string.Empty);
			try
			{
